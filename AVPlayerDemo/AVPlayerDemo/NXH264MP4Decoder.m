...
@implementation NXH264MP4Decoder

- (instancetype) initWithVideoSize:(CGSize)videoSize ofExportPath:(NSString *)exportPath {
    if (self = [super init]) {
        _videoSize = videoSize;
        //_filePath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"h264toMp4.mov"];
        _filePath = [exportPath copy];
    }
    return self;
}
...
- (void)pushH264Data:(unsigned char *)dataBuffer
              length:(uint32_t)len
            isIFrame:(BOOL)isIFrame
          timeOffset:(int64_t)timestamp {
...
- (CMSampleBufferRef)sampleBufferWithData:(NSData*)data
                         formatDescriptor:(CMFormatDescriptionRef)formatDescription {
    OSStatus result;
